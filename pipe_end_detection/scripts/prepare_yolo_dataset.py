...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Annotation status JSON. Bad warps are excluded by default when this file exists.",
    )
    parser.add_argument(
        "--include-bad",
        action="store_true",
        help="Include images marked as bad_warp in the status JSON.",
    )
...
def load_bad_warp_set(status_path: Path) -> set[str]:
    ...
    bad_warp = data.get("bad_warp", {}) if isinstance(data, dict) else {}
    return {str(key).replace("\\", "/") for key, value in bad_warp.items() if value}
...
def collect_samples(
    ...
    for image_path in iter_images(images_root):
        ...
        if relative_key in bad_warp_set:
            continue
...
def main() -> None:
    ...
    if bad_warp_set:
        print(f"- excluded bad warps: {len(bad_warp_set)}")
